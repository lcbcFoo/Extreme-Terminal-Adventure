...
void comandList() {
    ...
    mvprintw(1, 0,
             "w - anda para cima/ataca inimigo que esteja na posicao de cima "
             "no mapa;");
    mvprintw(2, 0,
             "d - anda para direita/ataca inimigo que esteja na posicao da "
             "direita no mapa;");
    mvprintw(3, 0,
             "s - anda para baixo/ataca inimigo que esteja na posicao de baixo "
             "no mapa;");
    mvprintw(4, 0,
             "a - anda para esquerda/ataca inimigo que esteja na posicao da "
             "esquerda do mapa;");
    mvprintw(5, 0, "b - abre o inventario;");
    mvprintw(6, 0,
             "c - mostra os atributos do jogador e permite que o jogador "
             "distribua os pontos;");
...
int menu(Nivel nivel, Player *player, Enemy *enemies, Bag *bag) {
    ...
    mvprintw(
        14, 0,
        "q - deixar o jogo (tenha certeza que voce salvou o jogo antes!)\n");
    ...
    do {
        ...
        if (get == 's') {
            if (saveGame(nivel, player, enemies, bag)) {
                ...
                mvprintw(5, 0,
                         "O jogo nao pode ser salvo, tente novamente\n\n");
...
void usaPot(Bag *bag, Player *player) {
    ...
    if ((*bag).quantidade == 0) (*bag).used = 0;
}
...
void printBag(Bag *bag, Player *player, Nivel *nivel) {
    ...
    do {
        ...
        if (get == 'r') {
            ...
            if ((index - '1' >= 0) && (index - '1' < TAM_BAG) &&
                (bag[index - '1'].used)) {
                if (nivel->mapa[player->y - 1][player->x].used == 0) {
                    nivel->mapa[player->y - 1][player->x].used = 1;
                    nivel->mapa[player->y - 1][player->x].quantItems =
                        bag[index - '1'].quantidade;
                    nivel->mapa[player->y - 1][player->x].itemIndice =
                        bag[index - '1'].item.indice;
                    bag[index - '1'].used = 0;
                    attron(COLOR_PAIR(4));
                    ...
                    nivel->mapa[player->y][player->x - 1].quantItems =
                        bag[index - '1'].quantidade;
                    nivel->mapa[player->y][player->x - 1].itemIndice =
                        bag[index - '1'].item.indice;
                    bag[index - '1'].used = 0;
                    attron(COLOR_PAIR(4));
                    mvprintw(5, 0, "O item foi dropado!\n");
                    ...
                    attron(COLOR_PAIR(2));
                    mvprintw(5, 0,
                             "Nao foi possivel dropar o item nessa posicao do "
                             "mapa!\n");
                    attron(COLOR_PAIR(1));
                    printBag(bag, player, nivel);
                    return;
            ...
            if (bag[get - '1'].item.tipo == 3) {
                ...
                mvprintw(5, 0, "Voce utilizou uma %s e recuperou %d de vida!\n",
                         bag[get - '1'].item.nome, bag[get - '1'].item.valor);
                attron(COLOR_PAIR(1));
                printBag(bag, player, nivel);
                return;
            }
...
void nextNivel(Nivel *nivel, Player *player, Enemy **enemy) {
    if (player->chave) {
        int next = nivel->nivel;
        free(nivel);
        nivel = genNivel(next + 1, player, enemy);
        clear();
        attron(COLOR_PAIR(3));
        ...
        attron(COLOR_PAIR(2));
        printw(
            "Voce nao possui a chave desse nivel, elimine o Boss para "
            "consegui-la!\n");
        attron(COLOR_PAIR(1));
    }
}
...
void showStats(Player *player) {
    ...
    clear();
    mvprintw(0, 0,
             "Existem 3 tipos de atributos: constituicao, destreza e forca. "
             "Voce ganha 5 pontos para distribuir entre esses atributos ao "
             "evoluir seu personagem.\n");
    mvprintw(1, 0,
             "Colocar pontos em constituicao te garante mais vida, em destreza "
             "te garante maior chance de se esquivar de ataques e de dar "
             "acertos criticos, ");
    ...
    attron(COLOR_PAIR(3));
    mvprintw(11, 0, "Voce possui %d pontos a serem distribuidos",
             (*player).pontos);
    mvprintw(14, 0,
             "Comandos possiveis:\n1 - Adiciona 1 ponto em constituicao (caso "
             "voce possua pontos a serem distribuidos);\n2 - Adiciona 1 ponto "
             "em destreza (caso voce possua pontos a serem distribuidos)");
    mvprintw(17, 0,
             "3 - Adiciona 1 ponto em forca (GASTA 5 PONTOS!) (caso voce "
             "possua pontos a serem distribuidos);\nr - retorna ao jogo\n\n");
...
int executeComand(char command, Player *player, Nivel *nivel, Enemy *enemies,
                  Bag *bag, Item *itens) {
    /* Opens menu */
    if (command == '-') {
        clear();
    ...
    /* Combat/move commands*/
        ...
        /* If there is an enemy in the position => combat */
            ...
            combate(
                player,
                &enemies
                    [(*nivel).mapa[(*player).y][(*player).x - 1].enemyIndice],
        ...
        /* If there is an item in the position => take item */
            ...
            takeItem(
                player,
                itens[(*nivel).mapa[(*player).y][(*player).x - 1].itemIndice],
    ...
    /* Same thing to d, w, s */
        ...
        if ((*nivel).mapa[(*player).y - 1][(*player).x].used == 0) {
            ...
            combate(
                player,
                &enemies
                    [(*nivel).mapa[(*player).y - 1][(*player).x].enemyIndice],
                &(*nivel).mapa[(*player).y - 1][(*player).x]);

        else if ((*nivel).mapa[(*player).y - 1][(*player).x].itemIndice != -1)
            takeItem(
                player,
                itens[(*nivel).mapa[(*player).y - 1][(*player).x].itemIndice],
